...
class NotionDocsServer {
    ...
    async searchPages(query = "", filterType = "page") {
        const searchData = {
            query,
            filter: {
                value: filterType,
        ...
        const result = await this.makeRequest('POST', '/search', searchData);
    ...
    async getPageContent(pageId) {
        const result = await this.makeRequest('GET', `/blocks/${pageId}/children`);
        return result.results || [];
    }
    async createPage(parentId, title, properties, content) {
        const pageData = {
    ...
    async updatePage(pageId, properties) {
        ...
        return await this.makeRequest('PATCH', `/pages/${pageId}`, updateData);
    }
    ...
    async queryDatabase(databaseId, filters, sorts) {
        ...
        const result = await this.makeRequest('POST', `/databases/${databaseId}/query`, queryData);
        return result.results || [];
    }
    createDocumentationBlocks(docType, content) {
        const blocks = [];
        switch (docType) {
            case "api_endpoint":
                blocks.push({
    ...
    createContentBlock(contentType, content, language) {
        switch (contentType) {
            case "paragraph":
                return {
                    object: "block",
    ...
    try {
        switch (name) {
            ...
            case "query_documentation_database":
                const dbResults = await notionServer.queryDatabase(args.database_id, args.filters, args.sorts);
                return {
                    content: [
                        {
                            type: "text",
            ...
            case "add_content_to_page":
                const block = notionServer.createContentBlock(args.content_type, args.content, args.language);
